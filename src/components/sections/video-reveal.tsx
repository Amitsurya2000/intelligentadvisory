...
export function VideoReveal() {
  ...
  useEffect(() => {
    ...
    const init = { w: isMobile ? "62%" : "45%", mid: isMobile ? "80%" : "62%", end: isMobile ? "92%" : "75%" };
  ...
  return (
    <section ref={sectionRef} id="video" className="relative h-[150vh] md:h-[220vh]">
      {/* Pinned stage */}
      <div
        ref={pinRef}
        className="sticky top-0 grid h-[64vh] place-items-center overflow-hidden md:h-[78vh]"
      ...
      >
        ...
        {/* Keywords — only visible while the video is playing */}
        {KEYWORDS.map((k) => (
          <span
            key={k.text}
            className={`pointer-events-none absolute z-30 hidden font-display text-lg font-semibold uppercase tracking-[0.2em] text-white/70 transition-all duration-700 ease-out lg:block ${k.className} ${
        ...
        <div className="relative z-20 flex w-full max-w-[1500px] justify-center px-4">
          ...
          >
            <div
              ...
              className="group glass-strong relative aspect-video max-h-[60vh] w-[45%] overflow-hidden rounded-[28px] shadow-[0_40px_120px_-30px_rgba(0,0,0,0.8)] ring-1 ring-white/10 will-change-transform"
            >
              <video
                ref={videoRef}
                src="/vsl.mp4"
